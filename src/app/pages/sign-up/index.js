...
const SignUp = ({ setLoading, setLoadingMsg, currentFrame, setCurrentFrame }) => {
    ...
    return (
        ...
        >
            <div style={{ width: '640px', height: '100%', overflowY: 'auto', padding: '20px' }}>
                ...
                <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-start', alignItems: 'center', gap: '30px', flexDirection: 'column' }}>
                    <Input
                        tag={'Email'}
                        value={email}
                        onChange={
                    ...
                    <Input
                        ...
                        customEle={<GiConfirmed style={{ marginRight: '10px' }} fontSize={'22px'} color={password === passwordConfirm && password !== '' ? 'rgba(0,150,0,1)' : 'rgba(250,0,0, 1)'} />}
                    />
                    <Input
                        tag={'Name'}
                        value={name}
                    ...
                    <Input
                        ...
                        customEle={<GiConfirmed style={{ marginRight: '10px' }} fontSize={'22px'} color={((recomCode !== '' && authKey !== '') && recomCode === authKey) ? 'rgba(0,150,0,1)' : 'rgba(250,0,0, 1)'} />}
                    />
                    <Input
                        tag={'Pin number'}
                        value={pinNumber}
                            ...
                            onClick={
                                () => {
                                    if (!email.includes('@') || email.split('@')[0].length < 2 || email.split('@')[1].length < 3 || !email.split('@')[1].includes('.')) {
                                        ...
                                        return
                                    } else if (recomCode === '' || authKey === '' || recomCode !== authKey) {
                                    ...
                                    reqJoin({
                                        email, countryDial: countryCode, name, password, phone: phone.slice(countryCode.length, phone.length), pin: pinNumber,
                                        callback: (err, response) => {
                                            if (err) {
                                                console.log(err)
